...
デフォルト: `y_left=0, y_right=12` (チェンナイ観測の幅12m に対応、y は右方向に増加)。
...
- **x**: 縦方向 (道路進行方向、正が前方)
- **y**: 横方向 (正が右方向, 論文準拠 "y is increasing to the right")
- 位置は車両の **front-center** を基準
- 車両は **矩形** (Fig. 2)

**境界力の符号** (論文 式(11) 準拠):
- 左境界接近 → 右に押す (g > 0, y 増加方向) — '+' for left boundary
- 右境界接近 → 左に押す (g < 0, y 減少方向) — '-' for right boundary
...
### 座標系 (Issue #19) — 解決済み
座標系を論文準拠 (y は右方向に増加) に統一済み。境界力の符号も論文式(9), (11) に直接対応。
